...
class RuntimeEventProjector:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        *,
        ...
        max_chunk_delay_seconds: float = 0.075,
    ) -> None:
        if min_chunk_chars <= 0 or max_chunk_chars < min_chunk_chars:
            raise ValueError("Runtime chunk bounds are invalid.")
        if not 0.05 <= max_chunk_delay_seconds <= 0.1:
            raise ValueError(
                "Runtime chunk delay must be between 50ms and 100ms.",
            )
    ...
    def project(
        ...
        *,
        ...
        if parent_message_id in self.internal_parent_message_ids:
    ...
    def _flush(self, now: float, *, force: bool) -> list[dict[str, str]]:
        ...
        if self.buffer and (
            force
            or len(self.buffer) >= self.min_chunk_chars
            or deadline_reached
        ):
            emitted.append(
                self._pop_chunk(
