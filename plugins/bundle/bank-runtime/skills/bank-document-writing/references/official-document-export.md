...
用户已明确“直接生成初稿”时，沿用前文主题和文种，缺失的金额、费用期间、主送单位、申请部门及依据可标为【待填写】或【待核实】，不要再次要求补齐后才起草，也不要编造具体金额、合同或制度事实。请示初稿仍使用公文 content 对象和 request/official_document 交付计划。若工具提示 JSON 不完整，应重新提交完整 kind/layout_version/document 对象，不将公文改成普通 sections/paragraphs，也不把对象再次编码成字符串。

主送、落款单位和成文日期必须使用专用字段：document.recipients、document.signatory、document.date，不能放入 blocks 普通段落。主送由排版器顶格排在标题后，落款与日期由排版器在正文后右对齐；不要靠空格、换行或正文缩进模拟。初稿缺项分别填【主送单位待填写】、【申请单位待填写】、【成文日期待填写】，不使用×××占位，不擅自将当前日期或登录人的部门当成成文日期、申请单位。blocks只放正文标题、正文段落与表格。
...
生成 Word 时把完整正文直接放入 content 对象（paragraphs/sections 或指定公文结构），不要手工拼接或二次 JSON 编码。一次调用必须同时包含正文、文件信息和所需 delivery_plan，不能先调用只有文件名的半成品参数。收到 JSON 不完整或结构校验错误时，重新构造完整对象并正确转义正文引号，只允许修正重试一次；再次失败立即说明未生成，不重复播报准备生成，不换工具绕过校验。
