...
按本轮需求通过原生 Skill 阅读器读取 `bank-document-writing`（写作、润色、摘要、长文、公文 DOCX）、`bank-document-review`（基础/公文审核、复核、修订）或 `bank-document-qa`（资料问答和来源引用）。各技能主文包含完整执行规则，不要求文件工具读取插件 references。仍使用当前任务真实授权的材料、工具和身份；技能可见不代表新增执行权限。
