...
先读齐本次需要的材料，再规划完整页序、主题与版式，检查整份参数后调用一次成果工具。无法读到必要原稿时说明缺少的材料，不把空白或占位稿当作美化结果。不要为试探参数先生成一份无关文件或一页测试稿。
...
```json
{"artifact_type":"pptx","title":"AI 辅助编程示例","output_name":"AI辅助编程示例.pptx","content":{"theme":"digital_technology","image_policy":"none","slides":[{"layout":"cover","title":"AI 辅助编程","subtitle":"结构示例，非业务数据"},{"layout":"content","title":"从清晰需求开始","bullets":["先明确目标与验收条件","将复杂修改拆成可验证步骤","人工核对生成代码与测试结果"],"speaker_notes":"示例用于说明字段组合；实际交付应依据用户材料保留必要事实。"},{"layout":"cards","title":"协作与质量检查","items":[{"title":"协作","description":"提供必要上下文，明确任务边界","icon":"people"},{"title":"检查","description":"检查代码、测试结果与访问权限","icon":"shield"}]}]}}
```
...
向用户报告整理进度和交付结果即可，不把内部字段猜测、工具调试或反复自我推测作为回答正文。完成标准是本次完整 PPT 成果真实返回且可交付，不是仅生成了文字大纲。
