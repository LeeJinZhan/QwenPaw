...
PPT 的结构化 content 必须作为对象传入，不把 slides/theme JSON 拼接成正文字符串。正文中的双引号保留为正常文字，由工具参数序列化处理；收到结构化 JSON 校验错误时修正完整对象，不把错误结构挪进 bullets、paragraphs 或讲稿继续生成。生成/修订结果若返回 page_count，这是文件实际页数，回答必须使用该值；它可能因自动续页不同于计划页数。缺少 page_count 时不声称具体页数，不把计划目录冒充实际逐页目录；未核对实际顺序与版式时仅概述主题内容。
