...
def test_phase_survives_tools_and_is_not_final_or_reasoning():
    ...
    assert chunks == []  # a tool preamble must never flash in the answer body
    ...
    assert final == []
    assert [e['event'] for e in projector.project({'object': 'response', 'status': 'completed'})] == ['answer.chunk', 'answer.completed']
...
def test_cancel_discards_unclassified_native_text():
    projector = CompactEventProjector("task")
    assert projector.project({"object": "message", "type": "message", "id": "m", "content": "I will use the tool."}) == []
    events = projector.project({"object": "response", "status": "failed"})
    assert [e["event"] for e in events] == ["answer.failed"]
