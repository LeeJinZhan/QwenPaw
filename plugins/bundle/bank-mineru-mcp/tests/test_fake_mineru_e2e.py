...
async def test_native_mcp_call_uses_fake_mineru_and_reads_large_result(
    ...
    async def fake_mineru(request: httpx.Request) -> httpx.Response:
        requests.append(request.url.path)
        if request.url.path == "/health":
            return httpx.Response(200, json={"status": "healthy", "version": "fake"})
    ...
    mineru = MinerUHttpClient(
        settings,
        http_client=httpx.AsyncClient(transport=httpx.MockTransport(fake_mineru)),
    )
        ...
        async with streamablehttp_client(
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                parsed = await session.call_tool(
                    "parse_documents",
                    {
                        "documents": [{"file_id": "file_001", "file_ref": file_ref}],
                        "parse_method": "ocr",
                        "language": "zh",
                    },
                )
